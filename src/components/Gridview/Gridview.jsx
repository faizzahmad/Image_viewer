...
export default function Gridview({id}) {
  ...
  const imageaction = (action) => {
    let i = getimage.i;
    if(action === "next-img"){
      setgetimage({img : dataimages.map(item => {
        return item.img
      })[i + 1], i : i + 1 })
  ...
  return (
    <div>
     ...
     {
        getimage.img && 
       <div className='bpr_fullgallerlyimage2 p-10 lg:p-0'>
       ...
       <button onClick={() => imageaction()}  className=' text-theme-text-color text-3xl fixed  right-[10px]  top-[10px]'><FaXmark/></button>
        <img src={getimage.img} style={{width:"auto", maxHeight : "90%",  maxWidth: "100%",}}></img>
